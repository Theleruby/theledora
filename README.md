...
All images are updated daily at ~10AM UTC. Updates should be available by 10:30AM. The desktop version should automatically download and install updates in the background. The gamescope version will offer updates via the SteamOS update helper. You can also update to the latest version manually using `rpm-ostree upgrade`.
